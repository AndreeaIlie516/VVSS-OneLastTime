...
public class WhiteBoxTest {
    ...
    public void testInsertCow_PairsAtBeginningAndEnd() {
        List<Animal> animals = List.of(new Mouse(), new Cat(), new Dog(), new Dog(), new Cat(), new Dog());
        List<Integer> pairs = List.of(0, 3);
        List<Animal> expected = List.of(new Mouse(), new Cow(), new Cat(), new Dog(), new Dog(), new Cat(), new Cow(), new Dog());
        List<Animal> result = Main.InsertCow(new ArrayList<>(animals), pairs);
    ...
    public void testInsertCow_ConsecutivePairs() {
        List<Animal> animals = List.of(new Mouse(), new Cat(), new Dog());
        List<Integer> pairs = List.of(0, 1);
        List<Animal> expected = List.of(new Mouse(), new Cow(), new Cat(), new Cow(), new Dog());
        List<Animal> result = Main.InsertCow(new ArrayList<>(animals), pairs);
